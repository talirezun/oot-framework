...
1. Confirm the always-on machine is configured (Mac mini / NUC / Pi 5 with FDE + UPS) per Skill Pack S12 §4.2, and the model stack is up: LM Studio serving headless on `http://127.0.0.1:1234/v1` — the desktop app's own server on macOS, or the standalone `llmster` daemon on a headless Linux box — with `--context-length 32768` on the load (required; the default 4096 can't hold the agent's system prompt), plus OpenCode installed with the `lmstudio` provider — see [`../installer/agent-assisted/OPENCODE-SETUP.md`](../installer/agent-assisted/OPENCODE-SETUP.md). A keep-warm `--ttl` is fine for an **always-loaded server you drive interactively**; **scheduled fires must instead chain `lms unload --all; lms load … --ttl 900 -y &&` into the cron/plist line** (per OPENCODE-SETUP.md §h, the TTL/JIT trap — a keep-warm TTL expires before the next fire, LM Studio then JIT-reloads at the default 4096 and breaks the run, and a plain re-`lms load` spawns a duplicate `<model>:2` instance).
...
3. Extract the prompt body from `cloud/<R>.md` into a standalone `~/oot-framework/routines/privacy/<r>.prompt.md`. **Prepend a first line that tells the agent to read the routine's owner SKILL.md file(s) before doing anything** (this replaces the old per-invocation `--skill` flags — the prompt now loads its own skills). **Two unattended-mode prompt-authoring rules** (per OPENCODE-SETUP.md §h): write any Python to a script file with the edit tool and run it as a single-line command — a multiline `python3 -c` blob does not match OpenCode's `python3 *` allow glob (globs don't span newlines) and is auto-rejected; and never instruct `rm` (not allow-listed, so deletion fails unattended) — leave temp files unstaged rather than deleting them.
