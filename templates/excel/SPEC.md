...
**During a firm's operation**, the canonical `.xlsx` files live at **`firm/excel/*.xlsx` in the firm's own Brain GitHub repo**. The installer copies the framework's templates there once during onboarding; from that point forward the firm's copies are stateful — they accumulate real partner output, real Klarna scores, real treasury snapshots — and are mutated by the framework's scheduled Routines.

**Who writes them.**

- **Routines (R1, R2, R3, R4, R6, R7, R8)** — automated writes via openpyxl in code execution, signed-committed to the firm's Brain repo's `main` branch. Track-symmetric: cloud Routines (Claude Code Routines on Anthropic infrastructure) and privacy Routines (cron on the always-on machine) perform the same operation against the same repo. No Google Sheets, no native Drive write-in-place, no remote Excel MCP. See [`docs/internal/ADR-001-cloud-routine-excel-writeback.md`](../../docs/internal/ADR-001-cloud-routine-excel-writeback.md) for the decision rationale.
- **Humans (founders, partners)** — manual writes via any spreadsheet app (see "Spreadsheet viewers" below), then `git add / commit -S / push` to the Brain repo. The framework's discipline: every X-file mutation is a commit, no exceptions. R6's audit-trail discipline collapses into the same artefact for free.
