...
Use this checklist when onboarding a new partner. The 90-minute onboarding session covers steps 1-15; the partner's first week covers steps 16-25; the 30-day check-in covers steps 26-30.
...
- [ ] **7. Reward Species Declaration.** Open `templates/excel/reward-species-declaration.xlsx`. Add the partner's sheet. Walk through every column. Negotiate where the partner wants different values. Validate sums (variable weights + bonus splits both = 1.0). Save.
...
- [ ] **11. Bitwarden vault provisioned.** Founder runs `provisioning-script.sh <partner_id>` step 1 (or completes manually per `PROVISIONING-SPEC.md`). Partner receives one-time login token.
...
- [ ] **14. Google Workspace seat (cloud) / PollinationX read access (privacy).** Step 4 of provisioning script.
...
- [ ] **26. Review first 30 days of output.** Open `templates/excel/partner-output-ledger.xlsx` Output_Log filtered to the partner. Discuss: was the value-tier classification accurate? Anything mis-captured?
