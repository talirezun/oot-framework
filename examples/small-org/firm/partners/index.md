---
title: "Partners — Solunar Studio"
slug: partners/index
domain: firm
type: index
created: 2026-02-01
updated: 2026-04-30
authors: [mira-tek]
status: active
---

# Solunar Studio — Partners

## Roster

| Partner | Cohort | Reward species | Joined | Two-worlds |
|---|---|---|---|---|
| [[partners/mira-tek]] (founder) | full-time-partner | hybrid 60/30/10 | 2026-02-01 | agentic-engineer |
| [[partners/davor-krznar]] | full-time-partner | eat-what-you-kill | 2026-02-15 | agentic-engineer |
| [[partners/anya-gorska]] | project-specialist | hybrid 80/15/5 | 2026-03-01 | vibe-coder |

## Recent renegotiations

None.

## Quarterly check-ins

- 2026-Q1 — completed. See [[partners/mira-tek/check-ins/2026-Q1]] (and per-partner pages).
- 2026-Q2 — scheduled mid-July.

## Roles

- **BR facilitator rotation:** Mira (week-of-04-13), Davor (week-of-04-20), Anya (week-of-04-27).
- **Compensation accountable:** Mira (founder).
- **Counsel point of contact:** Mira (Slovenian counsel on retainer).

## Notes

3-partner org; the framework runs at minimum scale here. Frame: *"if it can't work at 3, the framework is wrong."*
