...
def build_x1() -> None:
    ...
    headers = [
        ...
        "computed_variable",
        "notes",
        "weight",  # column N — co-authorship weight in (0, 1], default 1.0 (ADR-005)
    ]
    ...
    # Sample data (3 rows)
    # Columns A-N; J=partner_multiplier, K/L are formulas (written below), N=weight=1.0
    sample_data = [
        (
            "OL-20260301-001",
            "2026-03-01",
            ...
            "pr_merged",
            "gh:1100",
            "[[partners/mira-tek/output-specs/2026-02-25--login-flow]]",
            "M",
            30,
            "No",
            1.0,
        ...
        (
            ...
            "review_completed",
            "gh:1108",
            "[[partners/davor-krznar/output-specs/2026-02-28--code-review-cohort]]",
            "L",
            0,
            "No",
            1.0,
        ...
        (
            ...
            "deal_closed",
            "Acme Corp Q1",
            "[[partners/anya-gorska/output-specs/2026-02-15--acme-deal]]",
            "S",
            0,
            "No",
            1.0,
    ...
    for row_idx in range(2, 5):
        ...
        ws.cell(row=row_idx, column=11, value=f"=VLOOKUP(G{row_idx},$O$2:$P$5,2,FALSE)")
        # L (computed_variable) = K * J * N * IF(I="Yes", 0, 1) — N shares the
        # envelope across co-authors (ADR-005).
        ws.cell(
            row=row_idx,
            column=12,
            value=f'=K{row_idx}*J{row_idx}*N{row_idx}*IF(I{row_idx}="Yes",0,1)',
        )

    # Named ranges
    wb.defined_names["output_log"] = DefinedName(name="output_log", attr_text="Output_Log!$A:$N")
    ...
    ws.conditional_formatting.add(
        "I2:I1000", CellIsRule(operator="equal", formula=['"Yes"'], fill=RED_FILL)
    )
    ...
    # Sample row
    ws2.append(["2026-03", "[SAMPLE]mira-tek", 4, 3100, 1371, None, "draft", None, None])
    ws2["F2"] = "=D2+E2"
    ...
    ws4.append(
        ["[SAMPLE]davor-krznar", "Davor Krznar", 30000, 1.0, "eat-what-you-kill", "2026-03-01"]
    )
    ...
    ws4.conditional_formatting.add(
        "F2:F1000", FormulaRule(formula=['AND(F2<>"",TODAY()-F2>35)'], fill=RED_FILL)
    )
...
CO-AUTHORSHIP WEIGHT — column N (ADR-005)
Column N (weight) is a number in (0, 1], default 1.0. For a co-authored output R1 writes one row per co-author with weight = 1/N (or the output spec's attribution_split fractions). The L formula (=K*J*N*IF(rework)) shares the envelope across co-authors instead of paying each 100%. Two co-authors on one L-tier output receive €250 each, not €500 each.
...
def build_x2() -> None:
    ...
    dv_cohort = DataValidation(
        type="list", formula1='"full-time-partner,project-specialist,advisor"', allow_blank=False
    )
    ...
    # Leading partner_id join key (ADR-005). All prior columns shift right by one:
    # reward_species A→B, base_amount B→C, variable weights C/D/E→D/E/F,
    # output_multiplier F→G, bonus splits G/H/I→H/I/J.
    ws2 = wb.create_sheet("Base_Variable_Split")
    bvs_headers = [
        "partner_id",
        "reward_species",
        "base_amount",
        "variable_weight_personal",
        "variable_weight_team",
        "variable_weight_company",
        "output_multiplier",
        "bonus_split_personal",
        "bonus_split_team",
        "bonus_split_company",
    ]
    for col_idx, h in enumerate(bvs_headers, start=1):
        ws2.cell(row=1, column=col_idx, value=h)
    # Single-partner sample firm → partner_id P-001 (backfill rule, ADR-005 §3).
    ws2.append(["P-001", "hybrid", 30000, 0.6, 0.3, 0.1, 1.0, 0.34, 0.33, 0.33])

    # Validation: variable weights D+E+F = 1.0; bonus splits H+I+J = 1.0
    # Conditional formatting: highlight red if sum != 1.0 (ranges shifted +1 col
    # for the new leading partner_id column).
    # Guard with COUNT>0 so blank rows (sum 0) are not painted red — only populated rows.
    ws2.conditional_formatting.add(
        "D2:F1000",
        FormulaRule(formula=["AND(COUNT($D2:$F2)>0,ROUND(SUM($D2:$F2),3)<>1)"], fill=RED_FILL),
    )
    ws2.conditional_formatting.add(
        "H2:J1000",
        FormulaRule(formula=["AND(COUNT($H2:$J2)>0,ROUND(SUM($H2:$J2),3)<>1)"], fill=RED_FILL),
    )

    dv_species = DataValidation(
        type="list", formula1='"eat-what-you-kill,lockstep,hybrid"', allow_blank=False
    )
    ws2.add_data_validation(dv_species)
    dv_species.add("B2:B1000")  # reward_species shifted A→B

    _style_header_row(ws2, last_col=10)
    _set_column_widths(ws2, dict.fromkeys("ABCDEFGHIJ", 20))

    # ── Sheet 3: Long_Tail_Schedule ──
    # Leading partner_id join key (ADR-005); prior columns shift right by one.
    ...
    ws3.append(
        [
            "P-001",
            "[SAMPLE]OL-20260212-014",
            "Acme contract — 3yr ARR",
            8,
            "quarterly",
            "2026-04-01",
            "2029-03-31",
    ...
    ws4["A5"] = (
        "Fields locked / read-only in Gen 1. Activation requires written re-affirmation per the Charter."
    )
...
SCHEMA (ADR-005)
- Base_Variable_Split and Long_Tail_Schedule each carry a LEADING partner_id column (A, format P-NNN) that joins Partner_Profile column A. Single shared sheets, one row per partner (Base_Variable_Split) / per partner-artifact (Long_Tail_Schedule). R1/R3/R4 lookups join on partner_id.

VALIDATION
- Variable weights (Base_Variable_Split columns D+E+F) MUST sum to 1.0.
- Bonus splits (H+I+J) MUST sum to 1.0.
- Conditional formatting flags violations in red.
- Column G (output_multiplier) is NOT part of the variable-weight sum; it is a free-running multiplier.
...
def build_x3() -> None:
    ...
    _style_header_row(ws3, last_col=8)
    _set_column_widths(
        ws3, {"A": 16, "B": 12, "C": 50, "D": 18, "E": 30, "F": 30, "G": 50, "H": 14}
    )
    ...
    ws4.append(
        [
            "[SAMPLE]B-2026-04-001",
            "2026-04-15",
            "Acme legal review of long-tail clause",
            "anya-gorska",
            "anya-gorska",
            "open",
            None,
            "[[blockers/2026-04-15-acme-legal]]",
        ]
    )
    ...
    ws5.append(
        [
            "[SAMPLE]KT-2026-014",
            "Replace manual customer-onboarding email with AI drafter",
            "proceeded",
            16,
            "PROCEED",
            "2026-08-12",
...
def build_x4() -> None:
    ...
    dl_headers = [
        ...
        "trigger_ref",
        "scorer",
        "non_beneficiary_reviewer",
        "total_score",
        "decision",
        "reversal_plan_ref",
        "review_date_90d",
        "post_review_outcome",
        "status",  # column M — lifecycle enum, literal never formula (ADR-004)
    ]
    for col_idx, h in enumerate(dl_headers, start=1):
        ws.cell(row=1, column=col_idx, value=h)
    # M='monitoring': the sample test scored 20/20 and PROCEEDed, and is now in
    # its 90-day monitoring window (K = 2026-08-12 is in the future). I (decision)
    # says PROCEED; M (lifecycle) says monitoring — the ADR-004 transient
    # disagreement, demonstrated in the shipped sample.
    ws.append(
        [
            "[SAMPLE]KT-2026-014",
            "2026-04-29",
            ...
            "pr_label",
            "https://github.com/firm/firm-saas/pull/1502",
            "davor-krznar",
            "mira-tek",
            None,
            None,
    ...
    ws.conditional_formatting.add(
        "I2:I1000", CellIsRule(operator="equal", formula=['"PROCEED"'], fill=GREEN_FILL)
    )
    ws.conditional_formatting.add(
        "I2:I1000", CellIsRule(operator="equal", formula=['"HOLD"'], fill=RED_FILL)
    )
    ...
    ws.conditional_formatting.add(
        "K2:K1000", FormulaRule(formula=['AND(K2<>"",K2<TODAY(),L2="")'], fill=RED_FILL)
    )

    dv_trigger = DataValidation(
        type="list", formula1='"pr_label,manual,pre_rollout"', allow_blank=False
    )
    ws.add_data_validation(dv_trigger)
    dv_trigger.add("D2:D1000")

    # M (status) — lifecycle enum, literal never formula (ADR-004).
    dv_status = DataValidation(
        type="list",
        formula1='"scoring,remediation,monitoring,proceeded,held"',
        allow_blank=True,
        prompt="Lifecycle state (not the score verdict). R7 writes 'scoring' on create; "
        "transitions to proceeded/remediation/held then monitoring. See ADR-004.",
        promptTitle="Klarna lifecycle status",
    )
    ws.add_data_validation(dv_status)
    dv_status.add("M2:M1000")

    _style_header_row(ws, last_col=13)
    _set_column_widths(
        ws,
        {
            "A": 16,
    ...
    for col_letter in "BCDEFGHIJK":
        ...
        ws2.conditional_formatting.add(
            rng, CellIsRule(operator="equal", formula=["0"], fill=RED_FILL)
        )
        ws2.conditional_formatting.add(
            rng, CellIsRule(operator="equal", formula=["1"], fill=YELLOW_FILL)
        )
        ws2.conditional_formatting.add(
            rng, CellIsRule(operator="equal", formula=["2"], fill=GREEN_FILL)
        )
    ...
    ws2.conditional_formatting.add(
        "L2:L1000", FormulaRule(formula=['AND($A2<>"",L2<14)'], fill=RED_FILL)
    )
    ws2.conditional_formatting.add(
        "L2:L1000", FormulaRule(formula=['AND($A2<>"",L2>=14)'], fill=GREEN_FILL)
    )
    ...
    ws3.conditional_formatting.add(
        "F2:F1000", CellIsRule(operator="equal", formula=['"BREACHED"'], fill=RED_FILL)
    )
...
SCHEMA — decision (I) vs status (M) (ADR-004)
- Decision_Log column I (decision) is the THRESHOLD VERDICT: =IF(H>=14,"PROCEED","HOLD"), formula-driven.
- Decision_Log column M (status) is the LIFECYCLE STATE: a literal enum (scoring | remediation | monitoring | proceeded | held), never a formula. R7 writes 'scoring' on create; the scoring outcome moves it to proceeded/remediation/held; proceeded rows become 'monitoring' until the K review date passes.
- I and M may disagree transiently (e.g. I=PROCEED while M=monitoring during the 90-day window). The oot/klarna-test gate reads Klarna_Score directly and never consults status.
...
- Decision_Log column M (status) accepts only the five lifecycle enum values.
...
def build_x5() -> None:
    ...
    canonical_metrics = [
        ...
        (
            "[SPACE] performance_metric",
            "Custom domain metric",
            None,
            "2026-02-28",
            90,
            "tomislav-novak",
            "domain-specific; populate per-firm",
        ),
        (
        ...
        (
            "[DX Core 4] ai_assist_uptake",
            "Self-report weekly",
            0.18,
            "2026-02-28",
            90,
            "tomislav-novak",
            "fraction of commits AI-assisted",
        ),
    ]
    ...
    for row_idx in range(2, 6):
        ...
        ws2.cell(row=row_idx, column=6, value=f'=IF(ABS(E{row_idx})>20,"PERCEPTION_GAP","OK")')
    ws2.conditional_formatting.add(
        "F2:F1000", CellIsRule(operator="equal", formula=['"PERCEPTION_GAP"'], fill=RED_FILL)
    )
    # Comment in F1 about threshold provenance
    ws2["G1"] = "Threshold provenance"
    ws2["G2"] = (
        "20-point threshold ≈ half the 39-point swing observed in METR (2025) "
    ...
    dv_role = DataValidation(
        type="list", formula1='"participant,control,champion-candidate"', allow_blank=False
    )
    ...
    ac_headers = ["week_number", "active_users_pct", "completion_rate", "satisfaction_score"]
...
def build_x6() -> None:
    ...
    sample_costs = [
        (
            "2026-04-15",
            "claude-sonnet (compensation-attribution)",
            12500,
            3200,
            8,
            0.42,
        ...
        (
            "2026-04-15",
            "gemini-flash-lite (curator ingest)",
            45000,
            200,
            12,
            0.04,
            "S1 daily Brain ingest",
        ),
    ]
    ...
    har_headers = [
        ...
        "ai_agent_count",
        "agent_human_ratio",
        "monthly_outputs_human",
        "monthly_outputs_agent_assisted",
        "ratio_trend",
    ]
...
def build_x7() -> None:
    ...
    sample_arm = [
        (
            "[SAMPLE]UC-001",
            "none",
            "Internal engineering tooling; no decisions affecting persons.",
            "minimal",
            "approved",
            "2026-02-15",
        ),
        (
            "[SAMPLE]UC-002",
            "Annex III §4 (employment/access)",
            "Touches customer-onboarding decisions; counsel ruled limited-risk on basis of human escalation path.",
            "limited",
            "approved",
            "2026-04-22",
        ),
        (
            ...
            "Customer-facing chatbot; transparency obligation only.",
            "limited",
            "approved",
            "2026-01-30",
        ),
    ...
    dv_tier = DataValidation(
        type="list", formula1='"high,limited,minimal,prohibited"', allow_blank=False
    )
    ws2.add_data_validation(dv_tier)
    dv_tier.add("D2:D1000")
    dv_review = DataValidation(
        type="list", formula1='"pending,approved,objected,withdrawn"', allow_blank=False
    )
    ...
    dv_article = DataValidation(
        type="list",
        formula1='"compliant,partial,not-started,n/a (limited-risk),n/a (minimal-risk)"',
        allow_blank=True,
    )
    ...
    dv_evtype = DataValidation(
        type="list",
        formula1='"skill_pack_ref,routine_id,brain_page,risk_register_row,external"',
        allow_blank=False,
    )
...
def build_x8() -> None:
    ...
    ws2.append(
        ["2026-07-01", "Q2 long-tail settlement (forecast)", 12000, "EUR", "yes", "long-tail"]
    )
    ...
    dv_cat = DataValidation(
        type="list", formula1='"payroll,variable,long-tail,suppliers,other"', allow_blank=False
    )
    ...
    rc_headers = [
        "snapshot_date",
        "total_cash_eur",
        "monthly_burn_average",
        "runway_months",
        "unit_fund_outstanding_units",
        "implied_redemption_value",
    ...
    ws3.conditional_formatting.add(
        "D2:D1000", FormulaRule(formula=['AND($A2<>"",D2<6)'], fill=RED_FILL)
    )
    # G red if <1.0 — same populated-row guard.
    ws3.conditional_formatting.add(
        "G2:G1000", FormulaRule(formula=['AND($A2<>"",G2<1)'], fill=RED_FILL)
    )
    # Comment / note about burn-average source
    ws3["H1"] = "Note"
    ws3["H2"] = (
        "monthly_burn_average is derived from rolling 3-month Cash_Position deltas "
...
def build_x9() -> None:
    ...
    recs = [
        (
            "People",
            "If <60%: defer adoption until written partnership-buy-in exists and reward-species alignment is documented. See Skill Pack S3 §4.1.",
        ),
        (
            "Tech",
            "If <60%: complete the Curator install + Bitwarden + Trezor procurement before adoption. See QUICKSTART.md and governance/SECRETS-POLICY.md.",
        ),
        (
            "Culture",
            "If <60%: run a 1-week Klarna Test framing workshop. Read MANIFESTO.md Thesis 1 together. Cultural readiness is the bottleneck, not technical.",
        ),
        (
            "Risk",
            "If <60%: engage local counsel before any partner contract is signed. See docs/06-when-to-call-a-lawyer.md.",
        ),
    ]
    for row in recs:
        ws3.append(row)
