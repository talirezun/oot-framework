...
ØØT ships **8 scheduled Routines**. The cloud-track equivalents run on **Anthropic Remote Routines** — they execute on Anthropic's infrastructure, so they fire whether or not your laptop is open. The Routine prompts are markdown files at `routines/cloud/R1.md` through `R8.md`; you upload them to the Anthropic Routines dashboard.
...
1. **Open the Anthropic Routines dashboard.** [claude.com/routines](https://claude.com/routines) (or via Settings → Routines in Claude Desktop).
2. Click **"New Routine"**.
...
1. Anthropic Routines dashboard → **"New Routine"**.
...
6. **Connectors:** GitHub, Excel/Sheets, Slack.
7. Save. Manual fire (will populate X3 with the current partial-week's data).
8. **Verify:** X3 Weekly_Review has a new row; Slack `#business-review` has a draft summary.
...
- Cause: my-curator MCP is configured for Claude Desktop but the Routine on Anthropic's infrastructure can't reach your local Curator app.
- Fix: the Curator must be running on a network-reachable machine. If your laptop is closed, the Curator can't be reached. Privacy-track partners use the always-on machine; cloud-track partners can use Anthropic's Curator-as-a-service offering (Q3 2026 launch) or run the Curator on a small always-on Raspberry Pi.
